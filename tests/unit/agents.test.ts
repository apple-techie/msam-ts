...
describe("agents/registry", () => {
  it("GATEWAY_MAP maps agent IDs to gateway labels", () => {
    expect(GATEWAY_MAP["agent-a1"]).toBe("Gateway One (host-a + host-b)");
    expect(GATEWAY_MAP["main"]).toBe("Gateway One (host-a)");
    expect(GATEWAY_MAP["agent-b1"]).toBe("Gateway Two (host-c)");
  });

  it("GATEWAY_ORDER defines sorting", () => {
    expect(GATEWAY_ORDER["Gateway One (host-a + host-b)"]).toBe(0);
    expect(GATEWAY_ORDER["Gateway One (host-a)"]).toBe(1);
    expect(GATEWAY_ORDER["Gateway Two (host-c)"]).toBe(2);
  });
  ...
  it("EXCLUDE_AGENTS filters stale agents", () => {
    ...
    expect(EXCLUDE_AGENTS.has("worker")).toBe(true);
    expect(EXCLUDE_AGENTS.has("agent-a1")).toBe(false);
    expect(EXCLUDE_AGENTS.has("agent-b1")).toBe(false);
  });
