...
describe("pgvector integration", () => {
  it("stores and retrieves a vector", async () => {
    ...
    await pool.query(
      ...
      ["test-1", "test content", "hash1", "gateway-one", "semantic", "standard", "active", vectorToText(vec)]
    );
  ...
  it("performs cosine similarity search", async () => {
    ...
    await pool.query(
      ...
      [
        "atom-target", "target content", "h1", "gateway-one", "semantic", "standard", "active", vectorToText(target),
        "atom-similar", "similar content", "h2", "gateway-one", "semantic", "standard", "active", vectorToText(normalizedSimilar),
        "atom-ortho", "orthogonal content", "h3", "gateway-one", "semantic", "standard", "active", vectorToText(orthogonal),
      ]
    );
    ...
    await pool.query(
      `INSERT INTO atoms (id, content, content_hash, agent_id, stream, profile, state, embedding)
       VALUES
         ...
         ($9, $10, $11, $12, $13, $14, $15, $16::vector)`,
      [
        "a1", "gateway-one atom", "h1", "gateway-one", "semantic", "standard", "active", vectorToText(vec),
        "a2", "gateway-two atom", "h2", "gateway-two", "semantic", "standard", "active", vectorToText(vec),
      ]
    );
  ...
  it("handles JSONB metadata", async () => {
    ...
    await pool.query(
      ...
      ["meta-1", "test", "hm", "gateway-one", "semantic", "standard", "active", JSON.stringify(meta)]
    );
  ...
  it("stores and queries triples", async () => {
    await pool.query(
      ...
      ["src-atom", "Person_One is the founder of Project_One", "ht", "gateway-one", "semantic", "standard", "active"]
    );

    await pool.query(
      `INSERT INTO triples (id, atom_id, subject, predicate, object, confidence, state)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      ["t1", "src-atom", "Person_One", "is_founder_of", "Project_One", 0.95, "active"]
    );
    ...
    expect(result.rows[0].object).toBe("Project_One");
  ...
  it("tracks access_log entries", async () => {
    await pool.query(
      `INSERT INTO atoms (id, content, content_hash, agent_id, stream, profile, state)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      ["log-atom", "test content", "hl", "gateway-one", "semantic", "standard", "active"]
    );
  ...
  it("enforces content_hash + agent_id uniqueness for active atoms", async () => {
    await pool.query(
      `INSERT INTO atoms (id, content, content_hash, agent_id, stream, profile, state)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      ["dup-1", "same content", "same_hash", "gateway-one", "semantic", "standard", "active"]
    );
    ...
    await expect(
      pool.query(
        `INSERT INTO atoms (id, content, content_hash, agent_id, stream, profile, state)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        ["dup-2", "same content", "same_hash", "gateway-one", "semantic", "standard", "active"]
      )
    ).rejects.toThrow();
    ...
    await pool.query(
      `INSERT INTO atoms (id, content, content_hash, agent_id, stream, profile, state)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      ["dup-3", "same content", "same_hash", "gateway-two", "semantic", "standard", "active"]
    );
  });
  ...
  it("performs IVFFlat indexed search at scale", async () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      params.push(
        `scale-${i}`, `content ${i}`, `hash-${i}`, "gateway-one",
