...
- Normalize entities: Title_Case with underscores (Person_One, not person_one/Person_One/PERSON_ONE)
...
Person_One is a Founder (or Person), not just "Person". An investor is an Investor, not a "Concept".
...
Examples:
(Person_One [Founder], is_founder_of, Project_One [Startup])
(Gateway_One_Gateway [Gateway], deployed_on, Host_B [Hardware])
(Stripe [SaaS], integrates_with, Gateway_One [Startup])
(System_Alpha [AI_Agent], manages, FB_Marketplace_Scan [Automation])
