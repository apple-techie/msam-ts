...
// Names that are unambiguously a known type
const KNOWN_TYPES = {
  "andrew_peltekci": "Person", "drew": "Person", "andrew": "Person",
  "ryan": "Person", "justin_walker": "Person", "mark_fulton": "Person",
  "sannidhya_sah": "Person",
  "kainotomic": "Organization", "enduru_ai": "Organization",
  "enduru": "Organization", "letsdisagree": "Organization",
  "vercel": "Technology", "stripe": "Technology", "clerk": "Technology",
  "hubspot": "Technology", "react": "Technology", "neo4j": "Technology",
  "aurora": "Person",
};
...
function inferType(name, evidence) {
  ...
  if (KNOWN_TYPES[lname]) return KNOWN_TYPES[lname];

  // 2. "FirstName LastName" or "First_Last" → Person (runs BEFORE predicate voting)
  if (/^[A-Z][a-z]+[_ ][A-Z][a-z]+$/.test(name)) return "Person";

  // 3. Role-aware vote across ALL triples this entity appears in
  let personScore = 0;
  let techScore = 0;
  let orgScore = 0;
  let conceptScore = 0;
